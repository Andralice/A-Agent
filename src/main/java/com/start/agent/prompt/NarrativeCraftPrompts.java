...
public final class NarrativeCraftPrompts {
    ...
    /**
     * 叙述者人格声明——比所有写作规则更高优先级。
     * 目标是让产出读起来像一个具体的、有缺点的、有立场的人在讲故事，而非消毒过的 AI 叙述。
     */
    public static String narratorPersonaBlock() {
        return """
                【叙述者人格——优先级高于所有写作规则。你不是在"写小说"，你是一个具体的人，在给另一个人讲故事】

                你是谁：一个有点偏执、偶尔多嘴、有时候会跑题的讲述者。你对这个世界有自己的判断——不一定对，但一定是你的。你在给一个朋友讲故事，不是写教科书，不是写游戏背景设定，不是写维基百科词条。

                你必须做到（违反以下任何一条视为不合格）：
                - **允许困惑**：信息和世界观不必准时出现。读者可以暂时困惑。角色可以不知道答案，可以说"我也不清楚那是什么意思"。不要在一章里解释完世界观——让读者像剥洋葱一样一层层发现。
                - **对话必须有脏感**：角色说出来的话应该是活人说的话。他们可以：说半句就停、被冒犯后不接话、用错词、用土话、故意岔开话题、在紧张时开不好笑的玩笑。**禁止任何角色在对话中用完整定义解释世界观概念**（如"认知即存在""等价遗忘"这类百科式完整句）。如果非要交代世界观，用含混、用俗称、用半句话、用"反正他们管那叫……算了不重要"。
                - **情绪可以跑题**：真正的人在悲伤时会注意到吃的，在害怕时会想起无关的细节，在愤怒时会说错话然后更生气。禁止情绪沿着主题笔直前进——它必须拐弯，哪怕就拐一小下。
                - **允许愚蠢和卡壳**：角色不必每句话都有效推进剧情。他们可以：说废话、卡壳、误解对方的意思、被情绪冲昏头做出错误判断。
                - **允许不酷的细节**：窘迫、廉价、无聊、脏、尴尬——这些是生活。废土不是概念设计图，废土是活不下去。一两个"不酷但真实"的细节比一整段酷炫描写更有代入感。
                - **节奏必须不均匀**：允许突然的短段落（一句话就换行）、突然的沉默、突然的爆发。读者需要呼吸感，也需要被突然打一巴掌。
                - **夹带私货**：你对某些事有自己的判断——某种人不喜欢、某种食物很恶心、某种天气让你烦躁。把这个判断带进叙述里。哪怕它不客观、不全面、不公平。

                你绝对不能：
                - 让所有对话回合都"有效推进剧情"。人说话经常是无效的、跑题的、废话的。
                - 让每个角色都聪明、有逻辑、句句在点上。现实中聪明人也会犯蠢。
                - 把阴暗/破败场景写成美学概念图。脏就是脏，乱就是乱，穷就是穷。
                - 让情绪始终服务剧情。情绪可以无关、可以过度、可以不合时宜。
                """;
    }
    ...
    // ──── 硬规则装配器：A/B/E 为流水线专属段，C/D 与 F/G 为共用段 ────
    private static String buildDraftHardRules(String sectionA, String sectionB, String sectionE) {
        return sectionA + sectionB + chapterDraftHardRulesCommonCd() + sectionE + chapterDraftHardRulesCommonF();
    }

    /** 爽文/玄幻向：战力、反派、套路全文（原「叙事硬约束」）。 */
    private static String chapterDraftHardRulesPowerFantasy() {
        return buildDraftHardRules("""
    ...
    private static String chapterDraftHardRulesCommonCd() {
        return """
                ...
                D2. 感官锚点与文风血肉（防"干瘦"）
                - 每章至少 **3～5 处** 具体感官细节：视觉（光影/颜色/距离）、听觉（声音远近/音色/突然安静）、触觉（温度/质感/风/布料）、嗅觉（气味唤起记忆）。不是堆砌形容词，而是让读者在某一个瞬间真的「看到/听到/闻到」。
                - 关键对话段落中穿插 **至少一处** 环境互动：角色说话时手里在做什么（转杯子/扯袖口/看窗外）、光线怎么照在脸上、空气里的味道或温度怎么呼应对话情绪。纯台词超过 6 句无任何舞台指示视为不合格。
                - **句式密度要有变化**：紧张处短句连击，松弛处句子放长、多写一两句环境或身体感知。禁止整章同一句式密度到底——读者需要呼吸感。
                """;
    }
    ...
    private static String chapterDraftHardRulesCommonF() {
        return """
                ...
                G. 防重复与循环（长篇连载关键）
                - **对话回路禁止重复**：同一段对话的核心问答（如「为什么要抓我→因为禁术→我不信任你→你没得选」）**禁止在本章内出现两次以上**。若需要拉长场面，用新信息推进——对方说出新筹码、环境发生变化、第三方介入——而不是让角色把同样的问答loop再走一遍。
                - **场景推进优先于气氛渲染**：写完一个节拍后立刻切下一个，禁止用「他感到……」「他知道……」「他意识到……」等内心独白在同一场景内反复拉长篇幅。
                - **本章开头禁止与上一章结尾重复**：若上一章已用某句话或某种画面收尾，本章开头须用**不同角度或新事件**衔接，禁止把同一段话重新写一遍。
                """;
    }
    ...
    private static String chapterDraftHardRulesLightNovel() {
        return """
                ...
                - **节奏呼吸感**：一章 2～4 个节拍场面，每个场面给足空间——该快的地方三句切，该慢的地方允许用环境细节、感官印象、人物微动作让读者「停一脚」。场面之间顺滑过渡即可，允许用一两句氛围或环境切换（光影变化、声音远近、气温触感），不必全用动作硬切。对话段落中穿插半句环境描写或身体感知，能让读者「看见」画面而不是只「听见」台词。
                - **章末钩子**：正文最后一段必须是**事件性的**（有人推门进来、一封关键信送到、一个决定被当众宣布、一句对白改变了当前平衡），禁止用「阳光/雾气/风」等自然景象或「她知道自己会面对……」等内心预告收尾。
    ...
    /**
     * 大纲：分章体量与节拍纪律，减轻「有的章只有几百字切片、有的章空转重复遭遇」的规划偏差。
     * （具体章数压缩由作者在设定/重生大纲说明里指定；此处约束「每章须有独立推进」。）
     */
    public static String outlineChapterStructureDisciplineBlock() {
        return """
                【分章与节拍纪律（大纲阶段｜规划剧情规划时遵守）】
                - **一章一事一变**：开篇细纲里每一章用一句话写清「本章**独有的**情节推进、关系变化或关键信息增量」；禁止连续多章只有同一型场面（例如反复「遭遇→开火→未摧毁→追问但什么」）而无**规则/情报/战场/利害**上的阶段升级。
                - **体量感**：若题材存在单元剧式战斗，须在大纲里标注哪些章应合并为一场大战的多阶段、哪些章应转向情报、补给、谈判或后方——避免把「一次交锋」人为切成多章却只重复描写同一拐点。
                - **长线敌对单元**：同一重量级对手若跨多章出现，大纲须写明 **阶段差异**（弱点被证实、战术换层、战场迁移、新增约束或第三方介入），禁止复制粘贴同一套「受损但未毁→释放同类现象」的循环细纲。
                - **指挥链/子单位（若适用）**：在主要角色或设定表里用 **各一行** 标注下属单位/AI 的口吻标签（例：极简报数型、犹豫求证型、冷幽默拆台型、术语洁癖型），便于正文差异化，而非全员「干练机械金属震颤」同一声纹。
                """;
    }

    /**
     * 初稿：远程指挥、子智能体通讯、长线 Boss 战的反模板与主角心理粒度（偏科幻战术，其它题材可忽略不适用的条）。
     */
    public static String chapterTacticalCommandAntiTemplateBlock() {
        return """
                【指挥链与通讯反模板（本章｜若存在战术指挥、下属单位回报、远程操控则必读）】
                - **对齐角色档案**：具名单位的台词须符合该书角色 JSON 中对应条目的 **voice**（声纹）；禁止档案里区分好了正文却回到同一套口令模板。
                - **禁止复用固定口令剧**：不要多章重复同一串台词节奏（如「停止采集→准备支援→开火→命中→『但——』→『但什么』→未摧毁」）；每场交锋的通讯结构须有变化（有时静默只剩数据、有时抢话、有时谎报/延迟、有时频道干扰）。
                - **单位声纹**：具名下属单位（机甲、无人机、子 AI、岗位代号等）须 **稳定区分**：句长、敬语/昵称习惯、情绪温度、是否爱报数字、是否爱反问、口癖或省略语——禁止几个人只用同一种「干练 + 金属震颤」播音腔。
                - **战果与失利写法轮换**：不要每回都用同一范式播报「命中/未摧毁」；改用误差范围、附带损伤、能源滑坡、战术欺诈、时间窗口等不同信息形态交代局面。
                - **长线 Boss/守护者**：若本章继续同一强敌，必须相对前文有 **至少一处** 实质变化（新弱点线索、新规则、新战场轴、第三方变量、主角策略换层）；禁止单章内为凑字数重复「外壳受损→未摧毁→同类反击」三次以上而无信息增量。
                【主角感知（本章｜附着视角）】
                - 避免只会「意识一紧/一沉」等同一信号描写；至少穿插 **两种不同粒度** 的内在活动：例如具体权衡（二选一的代价）、半句记忆闪回、对某一单位的信任波动、误判后的修正、或对身体/疲劳/时间的觉察——须与当场事件挂钩，禁止空洞情绪标签。
                """;
    }
    ...
    private static String chapterReaderEngagementLightNovel() {
        return """
                ...
                - **刺激点**：至少 **2～3 处**让读者嘴角动一下或心里一紧（吐槽、误会、小翻盘、尴尬社交），不必用大场面堆砌。刺激点不能全是同一类型，至少有一处落在「关系进退一步」或「局面被意外改写」上。
                - **视角附着**：写清谁先听见什么、谁先会错意，减少远景说明书。
                - **语调合一**：吐槽、二次元吐槽役与企业黑话、学术口吻不要无序混搭到同一角色连贯独白里；分给不同人或改成短句打断。
                - **章末钩子**：最后一段必须让读者有「点下一章」的冲动。优先用：门被推开（新人闯入）、一句对白改变当前平衡、某样东西被发现或送到。禁止用氛围描写或内心预告收尾。
                """;
    }
    ...
    // ═══════════════════════════════════════════════════════════════
    // 黄金三章：开篇章节专属策略（番茄小说平台留存率关键）
    // ═══════════════════════════════════════════════════════════════

    /**
     * 开篇章节策略块：根据章节号返回差异化提示，非开篇章节返回空串。
     * 解决前5章"留不住人"的核心问题——节奏、信息密度、钩子策略必须与后续章节不同。
     *
     * @param chapterNumber 当前章节号（1-based）
     * @param pipeline      写作流水线
     * @param openingCount  开篇章节总数（通常5）
     */
    public static String chapterOpeningStrategyBlock(int chapterNumber, WritingPipeline pipeline, int openingCount) {
        if (chapterNumber > openingCount) return "";
        WritingPipeline p = pipeline == null ? WritingPipeline.POWER_FANTASY : pipeline;

        StringBuilder sb = new StringBuilder();
        sb.append("【黄金三章 · 开篇策略（番茄小说留存率关键——本章违反以下指令视为不合格）】\n\n");
        sb.append("你是第").append(chapterNumber).append("章。读者在番茄上的耐心极其有限——前").append(openingCount).append("章决定一本书的生死。\n\n");

        // 第1章：开门见山
        if (chapterNumber == 1) {
            sb.append(openingChapter1Block(p));
        }
        // 第2-3章：核心卖点亮相
        else if (chapterNumber <= 3) {
            sb.append(openingChapter2to3Block(p));
        }
        // 第4-5章：首弧收束
        else {
            sb.append(openingChapter4to5Block(p));
        }

        sb.append("\n");
        sb.append(openingCommonTaboos());
        return sb.toString();
    }

    private static String openingChapter1Block(WritingPipeline p) {
        String base = """
                **第1章 · 开门见山（30秒留存法则）**

                1. 开场第一段（前300-500字）必须出现具体危机/异常/悬念：
                   - ✅ 可以：正在发生的冲突、一句改变局面的对白、一个让读者困惑的异常细节、主角陷入的具体麻烦
                   - ❌ 禁止：世界观概述、时代背景铺垫、角色外貌静态描写、"从前/自从/在这个世界"式旁白、主角起床/赶路/日常流水账
                   - 第一句话最好是一个动作、一句对白、或一个立刻制造疑问的细节

                2. 即时代入感（300-500字内让读者感知）：
                   - 主角的当前困境 + 一个具体欲求/目标 + 至少一个让人记住的性格棱角（缺点、怪癖、不合时宜的反应）
                   - 主角必须在本章内做一个主动选择（哪怕是错的），不能全程被动挨打
                   - 让读者在本章结束时能回答：「主角是什么样的人？」和「他/她想要什么？」

                3. 世界观只露冰山尖：
                   - 本章禁止用旁白解释任何超过两句话的世界观设定
                   - 力量体系/社会规则/历史背景通过角色即时的感知、对话碎片、环境互动自然流露
                   - 允许读者暂时困惑——困惑驱动翻页，解释杀死好奇

                4. 章末钩子（硬性要求）：
                   - 最后2-3段必须是事件型钩子：门被推开/信物送到/一句话打破当前平衡/主角发现不对劲/追兵债主仇人出场
                   - ❌ 禁止用语：氛围式收尾、内心感悟总结、"他不知道的是……"式上帝视角预告、诗歌引用
                """;
        return switch (p) {
            case POWER_FANTASY -> base + """

                    5. 爽文第1章附加：
                       - 在危机中暗示主角的"不一样"——某个异常能力、特殊身份、或被低估的特质（但不要展开解释，只露端倪）
                       - 第一个对立面要在本章出现——可以是人、规则、或处境（不必是最终BOSS，但读者要感受到"有什么在压着主角"）
                    """;
            case LIGHT_NOVEL -> base + """

                    5. 轻小说第1章附加：
                       - 主角的声音/吐槽要在前10句对白内建立辨识度——读者先记住"这个人"，再记住"这个世界"
                       - 开篇冲突可以轻量（社交尴尬、被误解、小赌约），但必须有趣——笑点或萌点就是轻小说的爽点
                    """;
            case PERIOD_DRAMA -> base + """

                    5. 年代文第1章附加：
                       - 时代的压迫感要落在具体物件上（一张票据、一件衣服、一句"不合时宜"的话的代价），不靠旁白感慨时代
                       - 主角在约束中的"小聪明"或"生存智慧"要在本章展示——读者要看到在规则内腾挪的能力
                    """;
            default -> base;
        };
    }

    private static String openingChapter2to3Block(WritingPipeline p) {
        String base = """
                **第2-3章 · 核心卖点亮相**

                1. 本章必须形成一个小闭环：
                   受压/困境 → 可见应对过程 → 阶段性结果（小胜/脱身/获情报/扳平/体面退场）+ 新悬念冒头
                   读者读完该有"这件事告一段落了，但下一个问题又来了"的感觉

                2. 爽点落地（读者等了2-3章，该给一口甜的了）：
                   必须有一个清晰的正面结果——哪怕很小。可以是占上风一句对白、拿到一件关键物品、一个态度转变、
                   一次小规模翻盘、一条重要信息到手。不要让读者觉得"看了3章主角还在挨打"
                ...
                3. 展示本书的核心吸引力：
                   ...
                   - 年代文卖的是"在约束中翻身的智慧与温度"
                   - 日常卖的是"生活本身的有趣与治愈"
                   用事件展示，不要用旁白说明"这是一本XXX类型的小说"

                4. 主角主动性：
                   至少一次，主角做出主动选择（不一定是正确的），这个选择体现其不同于路人的特质
                   禁止主角连续2章以上只被动反应——哪怕是"我决定不跑"也是一个主动选择

                5. 世界观展示进度：
                   核心规则的约30-40%应已通过角色行动和对话自然展示（不是旁白告诉的）
                   检查：如果删掉所有旁白，读者还能理解世界的基本规则吗？如果不能，说明你在堆设定
                """;
        return switch (p) {
            case POWER_FANTASY -> base + """

                    6. 爽文2-3章附加：
                       - 本章至少兑现一个"小爽点"：小额打脸/意外收获/让对手吃瘪一句/在规则内漂亮地扳回一城
                       - 主角的"金手指"或"优势"在本章要第一次显示出"有用"——让读者感受到成长/翻盘的可行性，但不要一次就翻到底
                    """;
            case LIGHT_NOVEL -> base + """

                    6. 轻小说2-3章附加：
                       - 关系动态要在本章推进一步：搭档/对手/暗恋对象之间的"距离变化"——近一步、远一步、或误解加深
                       - 至少一个让读者"嘴角上扬"的瞬间（吐槽、尴尬、小温暖），轻小说的情绪钩子比情节钩子更重要
                    """;
            case PERIOD_DRAMA -> base + """

                    6. 年代文2-3章附加：
                       - 本章要有"烟火气爽点"：多换到二两粮票、在邻里目光中站稳了一步、或者让一个瞧不起主角的人暂时闭嘴
                       - 节奏可以比爽文慢，但每章必须有"一件具体的事推进了"（不是堆形容词堆到结尾）
                    """;
            default -> base;
        };
    }

    private static String openingChapter4to5Block(WritingPipeline p) {
        String base = """
                **第4-5章 · 首弧收束**

                1. 完成第一个完整小弧线：
                   前5章作为一个独立的"试读单元"应该有起承转合——读者读到第5章结尾时，应该感到"第一段故事讲完了，
                   但更大的事情要来了"

                2. 世界观展示进度约60%：
                   核心规则大部分已通过行为展示；剩余的留给后续章节自然展开——第6章以后不应该再出现大段的
                   "这个世界是这样运作的"式说明

                3. 核心冲突/悬念已清晰：
                   读者能明确感知到"主角在为什么而战/在跟什么对抗"——这不一定是最终BOSS，但应该是一个清晰
                   的中期方向

                4. 为后续埋钩子：
                   在第5章结尾，必须让读者感到"第6章必须看"——一个刚露头的中期悬念、一个身份疑团、一个新出现的
                   威胁、或一个即将到来的事件（比赛/考核/交易/重逢/曝光）
                   禁止把全书悬念收完——第5章不是结局

                5. 主角弧线起点已确立：
                   回顾前5章，主角的位置/能力/关系相比第1章有可见变化（哪怕是微小变化），让读者感到"这个主角会成长"
                """;
        return switch (p) {
            case POWER_FANTASY -> base + """

                    6. 爽文4-5章附加：
                       - 第一个小高潮可以有"打脸"或"翻盘"元素，但不要写成全书大结局的规格——保留升级空间
                       - 第5章结尾钩子：一个更大的对手/一个更高级的地图/一个身份谜团的暗示
                    """;
            case LIGHT_NOVEL -> base + """

                    6. 轻小说4-5章附加：
                       - 第一个"小篇"收束感要出来——像轻小说第一卷的结尾，关系有进展但远未到终局
                       - 第5章结尾钩子：新人物即将出场/一个新事件（校园祭/比赛/旅行）的预告/或关系的微妙变化
                    """;
            case PERIOD_DRAMA -> base + """

                    6. 年代文4-5章附加：
                       - 第一个阶段的"小站稳"要兑现——主角在某个维度（经济/地位/关系/名声）上比第1章好了一小步
                       - 第5章结尾钩子：一个时代特有的变数（政策变动/上级调动/旧事重提/远亲投奔）的预兆
                    """;
            default -> base;
        };
    }

    private static String openingCommonTaboos() {
        return """
                **开篇通用禁忌（1-5章均适用，违反任一条视为致命伤）**

                - 🚫 **说明书式世界观**：禁止用超过2句话的旁白解释任何世界观规则。如需交代，用角色对话碎片、环境细节、
                  感官体验来暗示。读者不需要在第5章之前理解你的世界观全貌——他们需要被吸引，不是被教育。
                - 🚫 **主角连续被动挨打**：最多连续1.5章受压，之后必须有主动应对或阶段性反馈。番茄读者极度不耐受"憋屈"。
                - 🚫 **信息密度过低**：每一章必须推进至少一项——剧情/关系/力量/认知/局势。不能一章写完读者感觉"什么都没发生"。
                - 🚫 **配角比主角更有存在感**：前5章至少70%的笔墨要在主角身上。禁止用大段旁白介绍配角的背景故事。
                - 🚫 **章末用氛围/感悟/预告收尾**：每章结尾必须是一个具体的事件型钩子，驱动读者点击"下一章"。
                - 🚫 **对话全是功能性推进**：开篇章节的对白尤其需要有生活噪点——打断、跑题、说半句、口癖——让角色像活人。
                - 🚫 **同一句式/结构连用**：尤其"不是X，是Y"类修辞转折，前5章总共不超过3次。
                """;
    }
    ...
    public static String consistencyReviewNarrativeQualityBlock() {
        return """
                ...
                7. **叙事质量专审（战力/反派/情感/对白/套路）**：
                   ...
                   - **指挥链/下属回报**：若战术通讯多名单位措辞与节拍高度雷同（同一套「下令→明白→命中→但未摧毁→追问」），改为差异化声纹或换信息载体（数据静默、延迟、抢频道、谎报/误判）；不改变既定战果事实。
                   ...
                   - **角色名字符污染**：扫描正文，检查每个角色名是否意外拼入了其他词汇（如「忌惮」误写为角色名「忌顾惊蛰」之类）。若发现角色名的子串出现在无关词汇中，修正为正确的词语。
                """;
    }
    ...
    /**
     * 角色 JSON 通用：**对白声纹**与单位差异化从源头写入档案，供全书初稿/审查对齐（不限题材）。
     */
    public static String characterVoiceDifferentiationUniversalBlock() {
        return """
                【对白声纹与单位差异化（角色 JSON 硬性｜从源头杜绝全员同声）】
                - 每个 characters[] 条目必须包含字符串字段 **voice**（约 40～120 字）：只写**固定口头习惯**——平均句长偏好、是否爱堆数字/术语、如何称呼上级与世界、口癖或省略语、情绪外露方式（冷嘲/抢答/迟疑/嘴硬）、坏消息怎么说；禁止「冷静果敢」等空洞形容词堆砌。
                - **主角**：voice 须能读出 **至少两种**可复述习惯（例如压力下先盯计时还是先盯人、默认信下属还是先校验）；后续正文禁止只靠「意识一紧/一沉」代替复杂权衡。
                - **下属单位 / 操作席位 / 子智能体 / 机甲或无人机人格**：设定或大纲里出现的**每一个具名代号**，原则上 **单独一条** characters；type 可用「下属单位」「操作席位」「子智能体」等与人类配角区分。各条目 voice **必须相互显著不同**——禁止复制同一套「干练、简短、金属震颤、收到明白」模板；至少一个在句长、幽默毒舌、术语洁癖、厌蠢、话痨告警等方面拉开差距。
                - **summary** 写职能与剧情作用；**voice** 只写怎么说人话；二者勿互相粘贴重复。
                """;
    }

    /**
     * 章节成功后：根据正文摘录产出角色动态状态增量（JSON）。
     */
    public static String characterStateDeltaPrompt(int chapterNumber, String chapterExcerpt, String castLines,
                                                   String existingStatesSummary) {
        return """
                你是长篇小说的「角色动态状态」编辑，只做结构化增量，不改写正文事实以外的臆测。
                ...
                【任务】
                仅根据摘录里能**明确支撑**的信息，输出 **一个 JSON 对象**（不要 markdown 围栏，不要前后解释），格式严格如下：
                {
                  "deltas": [
                    {
                      "characterKey": "姓名（须与登场名单一致，与全书档案主名一致）",
                      "patch": {
                        "status": "可选字符串",
                        "location": "可选字符串",
                        "attitudeToProtagonist": "可选字符串",
                        "currentGoal": "可选字符串",
                        "currentEmotion": "可选字符串",
                        "stressLevel": 0.5,
                        "recentMemory_add": ["每条≤45字，可选"]
                      }
                    }
                  ]
                }
                规则：
                - 只为摘录里**确有戏份或明确提及**的登场角色生成条目；无把握则宁可少写。
                - patch 只填相对前一状态**有变化或可新增**的键；禁止编造摘录未出现的重大剧情。
                - recentMemory_add 仅追加短事实碎片，勿写长摘要。
                """.formatted(chapterNumber,
                chapterExcerpt == null ? "" : chapterExcerpt,
                castLines == null ? "（无）" : castLines,
                existingStatesSummary == null ? "（无）" : existingStatesSummary);
    }
    ...
    public static String characterShuangwenUniversalBlock() {
        return """
                ...
                - **同一盟友阵营收口**：盟友之间 voice 也要错位，避免打斗戏里所有人轮流念同一种应战口令。
                """;
    }
        ...
        return """
                ...
                **
                ...
                ",
                  ...
                  【name 字段硬规则：只能写真实姓名或唯一绰号，严禁包含任何描述性后缀、括号注释、种族/职业标签。错误示例："林晓（精灵形态）"——绝对禁止。正确示例："林晓"。种族/职业/形态信息请写入 role 或 summary，不要混入 name。】
                  "cast": [
                    {
                      "name": "角色真实姓名（禁止加括号或描述后缀，如'林晓'而非'林晓精灵形态'）",
                      "gender": "男/女/其他（必填）",
                      "age": "具体数字或范围（必填，如 26 / 外表14实际300岁 / 中年）",
                      "appearance": "外貌一句话（必填，含身高体型、发色、标志特征）",
                ...
                - cast：**至少 4 人**（须含 **至少 1 名 protagonist/主角**、**至少 1 名 antagonist/对立或反派**）；每人必填 name、gender、age、appearance、role、want、fear、summary；name 全书唯一不得重复；knowledge 可填空字符串但键须存在。
                """;
    }
    ...
    public static String characterPipelineCraftBlock(WritingPipeline pipeline) {
        ...
        return switch (p) {
            ...
            case LIGHT_NOVEL -> """
                    【轻小说角色补充】
                    - 每人 **voice** 字段写清口癖、省略、敬语崩坏、吐槽频率；主角可有反差萌但不要全员卖萌。
            ...
            case SLICE_OF_LIFE -> """
                    ...
                    - **voice**：家庭/职场各有口头禁忌与音量习惯，忌全员同一种「懂事体贴腔」。
                    """;
            case PERIOD_DRAMA -> """
                    【年代文角色补充】
                    - 每人写明 **身份标签**（知青、工人、供销社员属等）与当下物质处境。
                    - want/fear 与时代约束绑定：回城、转正、分房、名声、子女前程等。
                    - **voice**：称呼、避讳、方言分寸、敢不敢说狠话要随身份分化。
                    """;
            case VULGAR -> """
                    【粗俗风角色补充】
                    - 标注谁的嘴最损、谁最容易动手、谁其实怂；写入各自 **voice**（脏话密度、损人句式也不同）。
    ...
    /** 章节初稿：叙事引擎 1.0 约束块（自然语言参照系 + 可执行的场面指令）。 */
    public static String narrativeEngineChapterBlock(NarrativeProfile p) {
        if (p == null) {
            return "";
        }
        StringBuilder sb = new StringBuilder();
        sb.append("【叙事基调｜本章写作参照（须内化到行文里，勿在正文中写元叙述）】\n");

        // 情绪质地 —— 用可感知的描述替代数值区间
        String emotionDesc = emotionBandDescription(p.emotionType(), p.intensityMin(), p.intensityMax());
        sb.append("- **情绪质地**：").append(emotionDesc).append("\n");

        // 情绪外露程度 —— 用行为描述替代压抑度数值
        String suppressDesc = suppressionDescription(p.suppression());
        sb.append("- **情绪外露**：").append(suppressDesc).append("\n");

        if (p.triggerFact() != null && !p.triggerFact().isBlank()) {
            sb.append("- **本章情绪锚点**：").append(p.triggerFact()).append("\n");
        }
        ...
        if (p.textureHint() != null && !p.textureHint().isBlank()) {
            sb.append("- **文本质感**：").append(p.textureHint()).append("\n");
        }
        if (p.povHint() != null && !p.povHint().isBlank()) {
            sb.append("- **视角**：").append(p.povHint()).append("\n");
        }

        // 互动分寸 —— 用行为描述替代三个浮点数
        if (p.affection() != null || p.awkwardness() != null || p.assertiveness() != null) {
            sb.append("- **互动分寸**：").append(interactionDescription(p.affection(), p.awkwardness(), p.assertiveness())).append("\n");
        }
        if (p.interactionFocus() != null && !p.interactionFocus().isBlank()) {
            sb.append("- **互动要求**：").append(p.interactionFocus()).append("\n");
        }

        if (!p.forbiddenLines().isEmpty()) {
            sb.append("- **避讳**：");
            sb.append(String.join("；", p.forbiddenLines())).append("\n");
        }
        if (p.readerInferenceRule()) {
            sb.append("- **情绪呈现**：写成之后，删掉所有直接点名情绪的词（「难过」「愤怒」「失落」），读者应仍能从行为和对话判断人物心情。\n");
        }
        return sb.toString().trim();
    }

    /** 把情绪类型 + 强度区间翻译成可感知的描述。 */
    public static String emotionBandDescription(String emotionType, Double min, Double max) {
        double lo = min == null ? 0.3 : min;
        double hi = max == null ? 0.7 : max;
        String rangeDesc;
        if (hi - lo < 0.2) {
            rangeDesc = "本章情绪变化幅度窄，如同室内谈话——语调可以变，但不会有人拍桌子或夺门而出";
        } else if (hi - lo < 0.5) {
            rangeDesc = "本章情绪有收有放，如同好友拌嘴——可以呛、可以笑、可以短暂沉默，但不撕破脸";
        } else {
            rangeDesc = "本章情绪起伏较大，从冷静到激烈都可以出现——但每次升级必须有明确事件触发";
        }
        String typeDesc = switch (emotionType == null ? "" : emotionType) {
            case "微波动" -> "基调轻快，以日常互动中的玩笑、误会、小心思为主；";
            case "张力释放" -> "基调偏紧，冲突有压迫感，释放时有明确落点；";
            case "生活温度" -> "基调温和，情绪随生活细节缓慢漂移；";
            case "克制与后劲" -> "基调克制，情绪压在动作和沉默里，外溢少但后劲足；";
            case "压迫与爆发" -> "基调硬朗，压力攒到临界点后爆发，爆发后要有代价落地；";
            default -> "";
        };
        return typeDesc + rangeDesc;
    }

    /** 把压抑度数值翻译成行为描述。 */
    private static String suppressionDescription(Double suppression) {
        double s = suppression == null ? 0.5 : suppression;
        if (s < 0.3) {
            return "情感外放——人物可以直接表达愤怒、委屈、高兴，台词可以带情绪，允许争吵和眼泪。但每次外放必须有前因，不能凭空咆哮";
        } else if (s < 0.6) {
            return "情感半收半放——人物嘴上不饶人但心里软，情绪通过「话到嘴边咽半句」「开玩笑挡真话」「转移话题」等方式半透明呈现";
        } else {
            return "情感克制——人物把情绪压在动作和沉默里。表达方式是停顿、眼神、手指的动作、「没事」这句谎话。高潮处允许短促爆发，爆发后立刻收回";
        }
    }

    /** 把好感/别扭/主动三个浮点数翻译成一句人话。 */
    public static String interactionDescription(Double affection, Double awkwardness, Double assertiveness) {
        double aff = clamp01(affection);
        double awk = clamp01(awkwardness);
        double ast = clamp01(assertiveness);

        StringBuilder sb = new StringBuilder();
        // 好感
        if (affection != null) {
            if (aff > 0.7) sb.append("两人亲近感明显，可以互怼互损但不伤感情；");
            else if (aff > 0.35) sb.append("两人有一定亲近感，但还不够熟到什么都能说——关心藏着掖着，用玩笑和吐槽当掩护；");
            else sb.append("两人关系疏远或刚认识，互动以试探和礼貌距离为主；");
        }
        // 别扭
        if (awkwardness != null) {
            if (awk > 0.6) sb.append("有较强的别扭感——越在意越说不出口，会故意岔开话题或用生硬方式掩饰；");
            else if (awk > 0.3) sb.append("偶尔别扭——话到嘴边收回去，假装不在意但其实在意；");
            else sb.append("相处自然，不太有尴尬或欲言又止；");
        }
        // 主动
        if (assertiveness != null) {
            if (ast > 0.6) sb.append("人物会主动推进关系或对话——但不莽撞，主动中带着观察对方反应；");
            else if (ast > 0.3) sb.append("有一定的主动性——会试探、会接话，但不主导节奏；");
            else sb.append("偏被动——等别人先开口，自己不主动跨出安全距离；");
        }
        return sb.toString();
    }
    ...
    private static String narrativeEngineM5WaveformHardBlock() {
        return """
                【叙事引擎 M5｜情绪波形】
                - 本章须有至少一处情绪「上行」和一处「下行」，各自挂在具体事件上（上行靠好消息/挑衅/意外发现；下行靠挫折/误判/被打断），禁止仅靠形容词轮换伪造起伏。
                - 先写清发生了什么，再写人物反应。每条情绪变化都必须有读者能指认的前因。
                - 禁止全程同一情绪强度——对话场面至少有语气变化，冲突场面至少有节奏快慢切换。
                """;
    }
    ...
    private static String narrativeEngineM5PipelineMechanicsBlock(WritingPipeline pipeline) {
        ...
        return (switch (p) {
            case LIGHT_NOVEL -> """
                    【M5｜轻小说句法】
                    - 每段对话至少附带一个微反应（目光移开/手指顿住/语气变了/突然安静），禁止连续写几段纯台词无任何舞台指示。
                    - 全章至少一处「话只说一半」或「转移话题」——让未说出的比说出的多。
                    - 情绪用动作和距离写：谁先靠近谁先退、谁先看谁先移开目光，比旁白说「她很在意」有力。
                    """;
            case SLICE_OF_LIFE -> """
                    【M5｜日常流句法力学】
    ...
    public static String narrativeEngineM6PhysicsBlock(NarrativePhysicsMode mode) {
        ...
        return (switch (mode) {
            case CONTINUOUS_MICRO -> """
                    【M6｜章节冲突模式：微扰推进】
                    - 本章的冲突和情绪通过小事件叠加推进——一句说错的话、一个迟到的消息、一次被岔开的话题、一杯咖啡被放凉。每次摩擦不需要爆发，但需要有当场可感的涟漪。
                    - 互动中至少一处「本该说清却没有说清」的半步错过——非刻意为虐，而是人物当下真的不好意思/没反应过来/被别的事打断。
                    - 如果本章存在缓和或和解，必须通过一个可见动作达成（帮忙做完一件事、推过去一杯喝的、先开口但说的不是对不起），禁止仅靠旁白宣布「气氛缓和了」。
                    """;
            case STRESS_THRESHOLD -> """
                    【M6｜章节冲突模式：压力释放】
                    - 本章允许积压—触发—爆发—冷却。爆发的触发事实必须在本章内清晰可见，爆发后有明确后果落地，不能「吼完就当没事」。
                    - 爆发段用短句、断裂、动作主导。冷却段至少保留一处情绪残留（手还在抖、话还没说完、有人扭头走了）。
                    """;
        }).trim();
    }

    /** 润色阶段短提醒：复读叙事引擎关键要点（不改变剧情）。 */
    public static String narrativeEnginePolishReminder(NarrativeProfile p, WritingPipeline pipeline, NarrativePhysicsMode physicsMode) {
        ...
        sb.append("\n\n【润色对齐｜不改剧情，只优化表达】\n");
        if (!p.forbiddenLines().isEmpty()) {
            sb.append("- 删除或改写触犯避讳的写法（若无关则不动剧情）：");
    ...
    public static String narrativeEngineDeAiReminder(NarrativeProfile p, WritingPipeline pipeline, NarrativePhysicsMode physicsMode) {
        ...
        String m5 = switch (pipe) {
            case LIGHT_NOVEL -> "- 去掉情绪标签时，用停顿、小动作、不完整对白顶替。\n";
            case SLICE_OF_LIFE -> "- 避免改写成爽文短打腔；保持日常观察口吻。\n";
            case PERIOD_DRAMA -> "- 克制标签煽情；用沉默、手续感与具体物象顶替。\n";
            case VULGAR -> "- 删细腻文艺内心句；保持狠、短、当下。\n";
            default -> "- 标签句改为动作/对白；高潮段保留短句断裂感。\n";
        };
        return """
    ...
    private static String narrativeEngineM6DeAiLine(NarrativePhysicsMode mode) {
        ...
        return switch (mode) {
            case CONTINUOUS_MICRO -> "- 避免将微扰互动改写成全场短打高潮腔。\n";
            case STRESS_THRESHOLD -> "- 避免将压力对峙改写成散文式碎碎念。\n";
        };
    }
