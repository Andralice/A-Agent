...
public class QQMessageController {
    ...
    public QQMessageController(NovelAgentService agentService,
    ...
    public String handleQQMessage(@RequestBody String payload) {
        try {
            log.debug("【收到请求】原始payload: {}", payload);
            qqMessageFacade.handle(payload);
            return RESPONSE_SUCCESS;
        } catch (Exception e) {
            log.error("【❌ 异常】处理QQ消息时发生错误", e);
    ...
    public ResponseEntity<byte[]> exportNovel(@PathVariable Long novelId) {
        try {
            ...
            headers.setContentDispositionFormData("attachment",
                    URLEncoder.encode("小说_" + novelId + ".txt", StandardCharsets.UTF_8));
            headers.setContentLength(content.length);

            log.info("【📄 导出完成】小说导出成功 - 小说ID: {}, 文件大小: {} bytes", novelId, content.length);
    ...
    public String triggerCleanup(@RequestHeader(value = "X-Admin-Token", required = false) String requestToken) {
        if (!StringUtils.hasText(adminToken)) {
            log.warn("【🔧 管理指令】未配置 app.admin-token，拒绝执行清理任务");
            return "{\"status\":\"failed\",\"message\":\"管理接口未启用\"}";
        }

        if (!adminToken.equals(requestToken)) {
            log.warn("【🔧 管理指令】清理任务鉴权失败");
            return "{\"status\":\"failed\",\"message\":\"无权限\"}";
        }

        try {
            log.info("【🔧 管理指令】手动触发空白小说清理任务");
            int cleanedCount = cleanupService.manualCleanupEmptyNovels();
            return String.format("{\"status\":\"ok\",\"cleaned_count\":%d}", cleanedCount);
        } catch (Exception e) {
            log.error("【❌ 管理指令】清理任务触发失败", e);
            return "{\"status\":\"failed\",\"message\":\"清理任务执行失败\"}";
        }
    }
}
